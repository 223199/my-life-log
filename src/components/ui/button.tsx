...
export const Button: React.FC<Props> = ({ className = '', variant = 'default', size='md', ...rest }) => {
  const base = 'inline-flex items-center justify-center rounded-md border text-sm font-medium transition-colors focus:outline-none px-3 py-2'
  const styles = {
  ...
  return <button className={`${base} ${styles} ${sizeCls} ${className}`} {...rest} />
}
