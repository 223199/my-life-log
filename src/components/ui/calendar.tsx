...
export const Calendar: React.FC<Props> = ({
  ...
  const days = useMemo(() => {
    ...
    const first = addDays(som, -som.getDay())
        ...
        {days.map((d, i) => {
          ...
          return (
            <button
              key={i}
              onClick={() => onSelect?.(d)}
              className={`aspect-square rounded-lg border text-[10px] flex flex-col items-center justify-start p-1 ${
                isToday ? 'outline outline-2 outline-indigo-500' : ''
              } ${inMonth ? 'bg-white' : 'bg-white/70 opacity-60'}`}
            >
              {renderDay ? (
                renderDay(d)
              ) : (
